...
use_colored_logging(#{os_type := {win32, _}}) ->
    %% Neither erl.exe nor werl.exe seem to support ANSI color escape
    %% sequences and Lager still tries to use them.
    false;
use_colored_logging(#{log_levels := #{color := UseColor}}) ->
    UseColor;
